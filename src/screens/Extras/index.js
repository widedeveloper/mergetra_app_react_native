...
export default class Schedule extends Component {
    ...
    Next(){
        let extra = [];
        if(this.state.addone1) extra.push(1);
        if(this.state.addone2) extra.push(2);
        if(this.state.addone3) extra.push(3);
        if(this.state.addone4) extra.push(4);
        if(this.state.addone5) extra.push(5);
        const {ServiceData} = this.props.navigation.state.params;        
        ServiceData.extra = extra;
